...
namespace Heap
{
    class Tester
    {
        ...
        private class IntAscendingComparer : IComparer<int> //used for min heap
        {
            public int Compare(int A, int B)
            {
                return Math.Sign(A - B); // return -1 if A<B
            }
        }

        private class IntDescendingComparer : IComparer<int> //used for max heap
        {
            public int Compare(int A, int B)
            {
                return -1 * Math.Sign(A - B); //return -1 if A>B
            }
        }
